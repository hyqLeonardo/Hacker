...
def create_voting_dict(strlist):
    ...
    return {strlist[i][0] : [int(x) for x in strlist[i][3 : ]] for i in range(len(strlist))}
...
def most_similar(sen, voting_dict):
    ...
    similar = {sen_b : policy_compare(sen, sen_b, voting_dict) for sen_b in voting_dict if sen_b != sen}
    for sen_b in similar:
        if similar[sen_b] == max([similar[sen] for sen in similar]):
            name = sen_b
    return name
...
def least_similar(sen, voting_dict):
    ...
    similar = {sen_b : policy_compare(sen, sen_b, voting_dict) for sen_b in voting_dict if sen_b != sen}
    for sen_b in similar:
        if similar[sen_b] == min([similar[sen] for sen in similar]):
            name = sen_b
    return name
...
def find_average_similarity(sen, sen_set, voting_dict):
    ...
    Example:
        ...
        -0.5
    """
    return sum([policy_compare(sen, sen_b, voting_dict) for sen_b in sen_set if sen_b != sen]) / len(sen_set)

most_average_Democrat = 'Collins' # give the last name (or code that computes the last name)

"""
f = open('voting_record_dump109.txt')
vd = list(f)
demo = [x for x in vd if "D" in x]
vd = create_voting_dict(vd)
demo = create_voting_dict(demo)
mean = {sen : find_average_similarity(sen, demo, vd) for sen in vd}
for sen in mean:
	if mean[sen] == max([mean[sen] for sen in mean]):
		name = sen

'Biden' or 'Collins' both right due to omit sen from compare list or not
"""
...
def find_average_record(sen_set, voting_dict):
    ...
    """
    res = []
    for i in range(len(voting_dict[list(sen_set)[0]])):
        mean = sum([voting_dict[sen][i] for sen in sen_set])/len(sen_set)
        res.append(mean)
    return res

average_Democrat_record = [0.0, -0.08, 1.0, 0.86, 0.98, 0.02, -0.72, 0.8, 0.98, 0.86, 0.88, 0.64, 0.72, 0.98, -0.42, 0.94, 0.96, 0.96, -0.24, 0.98, 1.0, 1.0, 1.0, 0.68, -0.4, 0.98, -0.14, 0.08, 0.98, 0.88, 0.96, 0.98, 1.0, 1.0, 0.98, -0.2, 0.98, -0.28, 0.34, 0.9, 0.52, 0.92, -0.68, 1.0, 0.92, -0.16] # give the vector as a list
...
def bitter_rivals(voting_dict):
    ...
    Example: 
        ...
        True
    """
    compare = {(x, y) : policy_compare(x, y, voting_dict) for x in voting_dict for y in voting_dict}
    for pair in compare:
        if compare[pair] == min([compare[key] for key in compare]):
            res = pair
    return res
