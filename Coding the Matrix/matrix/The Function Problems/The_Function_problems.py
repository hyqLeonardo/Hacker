...
def tuple_sum(A, B):
    ...
    for i in range(len(A)):
        res.append((A[i][0] + B[i][0], A[i][1] + B[i][1]))
...
comprehension_with_row = [ row(p, 20) for p in range(0, 15)]

comprehension_without_row = [ [ p+i for i in range(20) ] for p in range(0, 15)]
