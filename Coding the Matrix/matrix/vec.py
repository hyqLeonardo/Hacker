...
def equal(u,v):
    ...
    for key in u.D:
        if key not in u.f:
            temp = v.f[key] == 0 if key in v.f else key not in v.f
        elif key not in v.f:
            temp = u.f[key] == 0 if key in u.f else key not in u.f
        elif key not in u.f and key not in v.f:
            pass
        else:
            temp = v.f[key] == u.f[key]
...
def add(u,v):
    ...
    for key in u.D:
        if key in u.f and key in v.f:
            function[key] = u.f[key] + v.f[key]
        elif key not in u.f and key in v.f:
            function[key] = 0 + v.f[key]
        elif key in u.f and key not in v.f:
            function[key] = u.f[key] + 0
        elif key not in u.f and key not in v.f:
            pass
...
def dot(u,v):
    ...
    return sum([ v.f[key] * u.f[key] for key in u.D if key in v.f and key in u.f])
...
def scalar_mul(v, alpha):
    ...
    True
    """
    function = { key : v.f[key] * alpha for key in v.f }

    if function == { key : 0 for key in v.f }:
        function = {}

    return Vec(v.D, function)
...
def neg(v):
    ...
    True
    """
    return Vec(v.D, { key : -1 * v.f[key] for key in v.f })
